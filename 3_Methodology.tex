...
We use a patch-based training approach to cope with the high memory requirements of the GANs and with the small size of the training set. For each pre-processed image, a single 3D patch is extracted from the image and is used as to feed the model. The body mask is used to limit patch sampling to the body region to avoid extracting patches from the background, and the patches are sampled randomly with a uniform random distribution over the body region. We use a patch size of 128$\times$128$\times$32 voxels. During paired training, corresponding patches from the same physical position are extracted from the FDG-PET, pCT and HX4-PET-reg images. If $f_A$ and $f_B$ denote the position of focal points of the patches corresponding to domain \textit{A} and domain \textit{B} images, then in this case, $f_B = f_A$. Figure \ref{fig:paired_patch_sampling} shows a simplified visualization of the paired patch sampling process.
...
In case of unpaired training, a more complex sampling strategy is used instead of sampling patches randomly and independently from domain \textit{A} and domain \textit{B} images. This is to ensure a form of ``weak pairing" between the two sets of patches -- the anatomical region covered in their patches from \textit{A} images is roughly similar to the region covered by patches taken from \textit{B} image(s). 
First, corresponding patches from the same randomly chosen location are extracted from FDG-PET and pCT. Let the location of the focal point of this patch be denoted as $f_A$. Then, exploiting the fact that all scans have the same patient orientation, an equivalent focal point $f_B$ is estimated for the HX4-PET-unreg (and its coupled ldCT-unreg, if using). $f_B$ is calculated such that its \textit{relative} position with respect to the volume size of the HX4-PET-unreg is same as the relative position of $f_A$ with respect to the volume size of FDG-PET/pCT. This is followed by defining a \textit{local sampling region} of size $l_x \times l_y \times l_z$ around $f_B$, then updating the $f_B$ by randomly sampling from this local sampling region. Let this new focal point be denoted as $f_B'$. Again, the condition is applied to $f_B'$ that it must lie within the patient's body region and not in the image background. This additional stochasticity is introduced in the process to account for possible variation in the body position and structure of \textit{A} and \textit{B} patients. Figure \ref{fig:unpaired_patch_sampling} shows a visual representation of the unpaired patch sampling process.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{figures/Data/patch_sampling/unpaired_patch_sampling.png}
    \caption{``weak pairing" ...}
    \label{fig:unpaired_patch_sampling}
\end{figure}

The local sampling region is specified as the fraction of the volume size of an HX4-PET-unreg image, and in our experiments, we use size 35\% $\times$ 35\% $\times$ 60\%. This relatively large size was set to provide sufficient freedom for a valid patch to be sampled from the full images which are themselves not very large in size. 
